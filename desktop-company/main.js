...
function loadConfig() {
  ...
  try {
    ...
    const activationScope = parsed.activationScope || {};
    const appKey = String(activationScope.appKey || fallback.activationScope.appKey || 'company').trim().toLowerCase() || 'company';
    const companyId = String(activationScope.companyId || fallback.activationScope.companyId || 'COMP-001').trim().toUpperCase();
    const packageName = String(activationScope.packageName || fallback.activationScope.packageName || 'com.driver.portal.company.desktop').trim();
...
function createMainWindow() {
  ...
  ipcMain.handle('api:request', async (_event, payload) => {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          message: body && body.message ? String(body.message) : `HTTP ${response.status}`
        };
      }
  ...
  mainWindow.loadFile(path.join(__dirname, 'company-dashboard.html'));
}
