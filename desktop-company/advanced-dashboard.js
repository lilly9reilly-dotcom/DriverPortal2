...
function initReportCharts() {
  ...
  const otherCost = Math.max(0, Math.round((fuelCost + maintenanceCost) * 0.05));
  ...
  if (ctx2) {
    ...
    dashboardState.charts.expenseDistribution = new Chart(ctx2, {
      type: 'doughnut',
      data: {
        labels: ['وقود', 'صيانة', 'أخرى'],
        datasets: [{
          data: [fuelCost, maintenanceCost, otherCost],
          backgroundColor: ['#667eea', '#4facfe', '#95a5a6']
        }]
      },
      options: { responsive: true, maintainAspectRatio: true }
    });
...
async function loadActivationCodesDesktop() {
  ...
  tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px;">جاري تحميل بيانات التفعيل...</td></tr>';
  try {
    ...
    if (/Unknown action:|action is not allowed/i.test(String(error))) {
      dashboardState.activations.mode = 'local';
      dashboardState.activations.rows = loadLocalActivationRows_();
      renderActivationCodesDesktop();
      await uiAlert('النشر الحالي لا يدعم API التفعيل. تم التحويل إلى وضع محلي مؤقت مع دعم إنشاء وتصدير الأكواد.', 'بوابة الشركات والتفعيل');
      return;
    }
    tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;padding:20px;color:#c0392b;">فشل تحميل أكواد التفعيل: ${escapeHtml(String(error))}</td></tr>`;
  }
}
...
function loadLocalActivationRows_() {
  try {
    const raw = localStorage.getItem('desktopActivationRows');
...
function saveLocalActivationRows_(rows) {
  localStorage.setItem('desktopActivationRows', JSON.stringify(Array.isArray(rows) ? rows : []));
}

function randomActivationCode_() {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  const part = (len) => Array.from({ length: len }, () => chars[Math.floor(Math.random() * chars.length)]).join('');
  return `C-${part(4)}-${part(4)}`;
}
...
function exportActivationCodesDesktop() {
  ...
  const lines = ['code,enabled,appKey,companyId,allowedPackage,bound,lastSeenAt,notes'];
  rows.forEach((item) => {
    lines.push([
      safeCsv(item.code),
...
function renderActivationCodesDesktop() {
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px;">لا توجد أكواد تفعيل</td></tr>';
    return;
  }
...
async function createActivationCodeDesktop() {
  ...
  try {
    if (dashboardState.activations.mode === 'local') {
      const rows = loadLocalActivationRows_();
      const finalCode = code || randomActivationCode_();
      rows.unshift({
        code: finalCode,
        enabled: true,
        appKey,
      ...
      saveLocalActivationRows_(rows);
      dashboardState.activations.rows = rows;
      document.getElementById('activationCodeInputDesktop').value = '';
      renderActivationCodesDesktop();
      await uiAlert(`تم إنشاء كود تفعيل محلي: ${finalCode}`, 'تم');
      return;
    }
...
async function toggleActivationCodeDesktop(code, currentlyEnabled) {
  ...
  try {
    if (dashboardState.activations.mode === 'local') {
      const rows = loadLocalActivationRows_().map((item) => {
        if (String(item.code || '') !== code) return item;
        return { ...item, enabled: !currentlyEnabled };
      });
      saveLocalActivationRows_(rows);
      dashboardState.activations.rows = rows;
      renderActivationCodesDesktop();
...
async function updateActivationScopeDesktop(code) {
  ...
  const companyId = prompt('معرف الشركة (اختياري)', String(item.companyId || ''));
  ...
  try {
    if (dashboardState.activations.mode === 'local') {
      const rows = loadLocalActivationRows_().map((row) => {
        ...
        return {
          ...
          packageName: String(allowedPackage || '').trim()
        };
      });
      saveLocalActivationRows_(rows);
      dashboardState.activations.rows = rows;
