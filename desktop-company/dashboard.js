...
async function loadDashboardData() {
  try {
    ...
    if (data) {
      ...
      updateRecentMaintenance(data.maintenance || data.maintenanceRecords || []);
...
function updateStats(data) {
  ...
  document.getElementById('maintenanceCount').textContent = formatNumber(data.maintenanceCount || (data.maintenance || []).length || 0);
  document.getElementById('netProfit').textContent = formatCurrency(data.netProfit || data.profit || 0);
}
...
function updateStatsDummy() {
  ...
  document.getElementById('maintenanceCount').textContent = '18';
  document.getElementById('netProfit').textContent = '42,000,000';
}
...
function updateRecentMaintenance(items) {
  const tbody = document.getElementById('recentMaintenanceBody');
  const rows = Array.isArray(items) ? items : [];

  if (!tbody) return;

  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 20px; color: #999">لا توجد بيانات</td></tr>';
    return;
  }

  tbody.innerHTML = rows.slice(0, 8).map((item) => `
    <tr>
      <td>${escapeHtml(item.vehicle || item.carNumber || '')}</td>
      <td>${escapeHtml(item.type || item.maintenanceType || '')}</td>
      <td>${escapeHtml(item.status || 'مجدولة')}</td>
      <td>${formatCurrency(item.cost || item.estimatedCost || 0)}</td>
      <td>${formatDate(item.date || item.dueDate || '')}</td>
    </tr>
  `).join('');
}

function updateRecentMaintenanceDummy() {
  updateRecentMaintenance([
    { vehicle: 'بغ 123', type: 'زيت وفلاتر', status: 'منجزة', cost: 85000, date: '2026-07-04 11:00' },
    { vehicle: 'بغ 456', type: 'إطارات', status: 'قيد المتابعة', cost: 240000, date: '2026-07-03 09:15' },
    { vehicle: 'بغ 789', type: 'كهرباء', status: 'مجدولة', cost: 65000, date: '2026-07-02 14:20' }
  ]);
}
...
function updateAlerts(items) {
  ...
  if (!rows.length) {
    box.innerHTML = `
      <div class="alert-card info"><i class="fas fa-circle-info"></i><div><strong>لا توجد تنبيهات حرجة</strong><p>النظام يعمل بشكل طبيعي.</p></div></div>
    `;
    return;
  }

  box.innerHTML = rows.slice(0, 4).map((item) => `
    <div class="alert-card ${item.level || 'info'}">
...
function updateAlertsDummy() {
  updateAlerts([
    { level: 'warning', icon: 'triangle-exclamation', title: 'وصلة مكررة', message: 'تم اكتشاف رقم وصلة بحاجة إلى مراجعة.' },
    { level: 'success', icon: 'circle-check', title: 'تحديث ناجح', message: 'آخر مزامنة مع الخادم تمت بنجاح.' }
  ]);
}
...
function initCharts(data) {
  ...
  if (monthlyChart && dashboardState.charts.monthly) {
    dashboardState.charts.monthly.destroy();
  }

  if (monthlyChart) {
    dashboardState.charts.monthly = new Chart(monthlyChart, {
      type: 'line',
      data: {
        labels: ['يناير', 'فبراير', 'مارس', 'أبريل', 'مايو', 'يونيو', 'يوليو'],
        datasets: [{
          label: 'الإيراد الشهري',
          data: [22, 28, 24, 31, 36, 40, 45],
          borderColor: '#0f766e',
      ...
      options: {
        ...
        maintainAspectRatio: true,
        plugins: {
          legend: { labels: { font: { family: "'Alyamama', 'Tajawal', Arial", size: 12 } } }
        },
        scales: {
          y: {
            beginAtZero: true,
