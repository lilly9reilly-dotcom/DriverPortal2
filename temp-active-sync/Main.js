...
function handleRequest(e) {
  ...
  try {
    ...
    if (isProtectedAction_(action)) {
      var scopeValidation = validateScopeForProtectedAction_(action, data);
      if (!scopeValidation.success) {
        return json(scopeValidation);
      }
    }
...
function validateScopeForProtectedAction_(action, data) {
  var companyId = normalizeScopeValue_(data && data.companyId);
  var activationCode = normalizeScopeValue_(data && data.activationCode);

  if (!companyId || !activationCode) {
    return {
      success: false,
...
function normalizeScopeValue_(value) {
...
function saveFactoryMain_(data) {
  ...
  try {
    ...
    var sendTime = Utilities.formatDate(new Date(), "Asia/Baghdad", "yyyy-MM-dd HH:mm:ss");
    var loadDateValue = data.loadDate || data.unloadDate || "";
    var unloadDateValue = data.unloadDate || data.loadDate || "";
    var factoryNotes = data.notes || "";
    var factoryFare = data.kroa || data.tripPrice || data.price || data.driverFare || "";
    if (factoryFare) {
      factoryNotes = (factoryNotes ? factoryNotes + "\n" : "") + "كروة السائق: " + factoryFare;
    }

    sheet.appendRow([
      rawDocNumber,
      data.driverName || "",
      data.carNumber || data.vehicleNumber || "",
      loadDateValue,
      unloadDateValue,
      data.quantity || 0,
      data.owner || data.ownerType || "",
...
function getDriverWallet(data) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var i = startRow; i < values.length; i++) {
      ...
      var rowPrice = toNumber_(getCellByAliases_(row, colMap, ["kroa", "driverfare", "tripprice", "fare", "سعرالنقل", "الكروه", "الكروة"], 13));
      if (rowPrice <= 0) {
        rowPrice = extractDriverFareFromNotes_(getCellByAliases_(row, colMap, ["notes", "note", "remarks", "ملاحظات"], 14));
      }
      ...
      var moneyQty = normalizeQuantityForMoney_(rowQty);
      var deductionQty = roundNumber_(moneyQty * DEDUCTION_RATE, 3);
      var netQty = roundNumber_(moneyQty - deductionQty, 3);
...
function getAllReceiptsData(month) {
  ...
  for (var s = 0; s < targetSheets.length; s++) {
    ...
    for (var i = startRow; i < values.length; i++) {
      ...
      var isFactorySheet = /^f_/i.test(sheetName);

      if (isFactorySheet) {
        var docNumberFactory = String(row[0] || "");
        var driverNameFactory = String(row[1] || "");
        var carNumberFactory = String(row[2] || "");

        var loadDateFactory = String(row[3] || "");
        var unloadDateFactory = String(row[4] || "");
        var quantityFactory = toNumber_(row[5]);
        var ownerFactory = String(row[6] || "");
        var destinationFactory = String(row[7] || "");
        var imageFactory = String(row[8] || "");
        var timestampFactory = String(row[9] || "");
        var vehicleOwnerFactory = String(row[10] || "");
        var notesFactory = String(row[11] || "");
        var sourceFactory = String(row[12] || "factory");

        // Legacy compact factory row shape:
        // [doc, driver, car, quantity, factory, imageUrl, unloadDate, ...]
        if (row.length <= 8 || (quantityFactory <= 0 && toNumber_(row[3]) > 0)) {
          quantityFactory = toNumber_(row[3]);
          destinationFactory = String(row[4] || destinationFactory || "");
          imageFactory = String(row[5] || imageFactory || "");
          unloadDateFactory = String(row[6] || unloadDateFactory || "");
          loadDateFactory = String(row[6] || loadDateFactory || "");
          if (!timestampFactory && row.length > 9) timestampFactory = String(row[9] || "");
          if (!notesFactory && row.length > 7) notesFactory = String(row[7] || "");
          sourceFactory = "factory";
        }

        if (!loadDateFactory && unloadDateFactory) loadDateFactory = unloadDateFactory;
        if (!unloadDateFactory && loadDateFactory) unloadDateFactory = loadDateFactory;
        if (toNumber_(loadDateFactory) > 0 && String(unloadDateFactory || "").trim() !== "") {
          loadDateFactory = unloadDateFactory;
        }

        if (ownerFactory.indexOf("GMT") > -1) {
          ownerFactory = "";
        }

        var displayFactoryQty = normalizeQuantityForMoney_(quantityFactory);
        var deductionFactoryQty = roundNumber_(displayFactoryQty * DEDUCTION_RATE, 3);
        var netFactoryQty = roundNumber_(displayFactoryQty - deductionFactoryQty, 3);
        var storedFactoryFare = toNumber_(extractDriverFareFromNotes_(notesFactory));
        var finalFactoryAmount = roundNumber_(netFactoryQty * getPricePerTonBySource_(true), 0);

        result.push({
          row: i + 1,
          sheetName: sheet.getName(),
          docNumber: docNumberFactory,
          driverName: driverNameFactory,
          carNumber: carNumberFactory,
          loadDate: loadDateFactory,
          unloadDate: unloadDateFactory,
          quantity: displayFactoryQty,
          deductionQty: deductionFactoryQty,
          netQuantity: netFactoryQty,
          owner: ownerFactory,
          destination: destinationFactory,
          imageUrl: imageFactory,
          timestamp: timestampFactory,
          vehicleOwner: vehicleOwnerFactory,
          liters: 0,
          bojer: "",
          distance: 0,
          kroa: storedFactoryFare,
          storedPrice: storedFactoryFare,
          price: finalFactoryAmount,
          gasCost: 0,
          notes: notesFactory,
          period15: getPeriodFromDate_(unloadDateFactory || loadDateFactory || timestampFactory),
          month: monthKey,
      ...
      var fallbackQtyIndex = isFactorySheet ? 5 : 5;
      var grossQty = toNumber_(getCellByAliases_(row, colMap, ["quantity", "qty", "الكمية"], fallbackQtyIndex));
      var displayQty = normalizeQuantityForMoney_(grossQty);
      var deductionQty = roundNumber_(displayQty * DEDUCTION_RATE, 3);
      var netQty = roundNumber_(displayQty - deductionQty, 3);
      ...
      var source = String(getCellByAliases_(row, colMap, ["source", "type", "rowtype"], -1) || "");
      var destination = String(getCellByAliases_(row, colMap, ["station", "destination", "المحطة", "الوجهة", "المحطة/المعمل", "المحطةالمعمل"], isFactorySheet ? 7 : 7) || "");
      var factory = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة"], isFactorySheet ? 7 : -1) || "");
      if (isFactorySheet && !destination && !factory) {
        // Legacy factory rows were saved with factory name at index 4.
        var legacyFactoryName = String(getCellByAliases_(row, colMap, ["unloaddate", "unload", "تاريخالتفريغ"], 4) || "");
        if (legacyFactoryName) {
          destination = legacyFactoryName;
          factory = legacyFactoryName;
        }
      }
      ...
      var notes = String(getCellByAliases_(row, colMap, ["notes", "note", "remarks", "ملاحظات"], 14) || "");
      var storedFare = toNumber_(getCellByAliases_(row, colMap, ["kroa", "driverfare", "tripprice", "fare", "سعرالنقل", "الكروه", "الكروة"], 13));
      if (storedFare <= 0) {
        storedFare = extractDriverFareFromNotes_(notes);
      }

      var loadDate = String(getCellByAliases_(row, colMap, ["loaddate", "load", "date", "تاريخالتحميل"], 3) || "");
      var unloadDate = String(getCellByAliases_(row, colMap, ["unloaddate", "unload", "تاريخالتفريغ", "filldate", "fill_date"], isFactorySheet ? 4 : 4) || "");
      ...
      if (!unloadDate && loadDate) unloadDate = loadDate;

      // In some legacy factory rows, numeric quantity leaked into loadDate fallback.
      // If loadDate is numeric and unloadDate is a date-like value, prefer unloadDate.
      if (isFactory && toNumber_(loadDate) > 0 && String(unloadDate || "").trim() !== "") {
        loadDate = unloadDate;
      }
      ...
      result.push({
        ...
        deductionQty: deductionQty,
        netQuantity: netQty,
        owner: String(getCellByAliases_(row, colMap, ["owner", "ownertype", "المالك"], isFactorySheet ? -1 : 6) || ""),
        destination: destination,
        imageUrl: String(getCellByAliases_(row, colMap, ["imageurl", "image", "photo", "رابطالصورة", "صوره"], isFactorySheet ? 8 : 8) || ""),
        timestamp: timestamp,
        vehicleOwner: String(getCellByAliases_(row, colMap, ["vehicleowner", "owner2", "مالكالسيارةأوالمالك", "مالكالسيارهاوالمالك"], isFactorySheet ? 10 : -1) || ""),
        liters: liters,
...
function repairFactorySheetLayout(data) {
  ...
  if (!monthKey) {
    monthKey = buildMonthKey_(new Date());
  }

  var sheetName = "F_" + monthKey;
  var headers = factoryHeaders_();
  var sheet = ss.getSheetByName(sheetName);
  if (!sheet) {
    sheet = ensureFactorySheet_(ss, sheetName);
    return {
      success: true,
      message: "تم إنشاء شيت المصنع بالتخطيط الجديد",
      month: monthKey,
      sheetName: sheetName,
      migratedRows: 0
    };
  ...
  for (var r = 1; r < values.length; r++) {
    var normalized = normalizeFactoryRowForLayout_(values[r] || []);
    if (isMeaningfulFactoryRow_(normalized)) {
  ...
  return {
    success: true,
    message: "تم إصلاح تخطيط شيت المصنع وترحيل الصفوف",
    month: monthKey,
    sheetName: sheetName,
...
function normalizeFactoryRowForLayout_(row) {
  row = row || [];

  var doc = String(row[0] || "").trim();
  var driver = String(row[1] || "").trim();
  var car = String(row[2] || "").trim();

  var loadDate = row[3] || "";
  var unloadDate = row[4] || "";
  var quantity = toNumber_(row[5]);
  var owner = String(row[6] || "").trim();
  var destination = String(row[7] || "").trim();
  var imageUrl = String(row[8] || "").trim();
  var sendTime = row[9] || "";
  var vehicleOwner = String(row[10] || "").trim();
  var notes = String(row[11] || "").trim();
  var source = String(row[12] || "").trim();

  var legacyQty = toNumber_(row[3]);
  var looksLegacy = quantity <= 0 && legacyQty > 0;
  if (looksLegacy) {
    quantity = legacyQty;
    destination = destination || String(row[4] || "").trim();
    imageUrl = imageUrl || String(row[5] || "").trim();
    unloadDate = unloadDate || row[6] || "";
    loadDate = loadDate || unloadDate || "";
    source = source || "factory";
  }

  if (loadDate && !isLikelyDateValue_(loadDate)) loadDate = "";
  if (unloadDate && !isLikelyDateValue_(unloadDate)) unloadDate = "";
  if (!loadDate && unloadDate) loadDate = unloadDate;
  if (!unloadDate && loadDate) unloadDate = loadDate;
  if (toNumber_(loadDate) > 0 && String(unloadDate || "").trim() !== "") {
    loadDate = unloadDate;
  }

  if (owner.indexOf("GMT") > -1) owner = "";
  if (!destination && String(row[4] || "").indexOf("معمل") > -1) {
    destination = String(row[4] || "").trim();
  }
  if (!destination && String(row[7] || "").indexOf("معمل") > -1) {
    destination = String(row[7] || "").trim();
  }

  if (String(loadDate || "").trim() === String(destination || "").trim()) {
    loadDate = "";
  }
  if (String(unloadDate || "").trim() === String(destination || "").trim()) {
    unloadDate = "";
  }

  if (!notes && String(row[10] || "").indexOf("سعر") > -1) {
    notes = String(row[10] || "").trim();
    vehicleOwner = "";
  }
  if (!notes && String(row[11] || "").indexOf("سعر") > -1) {
...
function isLikelyDateValue_(value) {
  ...
  if (!s) return false;
  if (s.indexOf("GMT") > -1) return true;
  if (/^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/.test(s)) return true;
  if (/^\d{1,2}[\/-]\d{1,2}[\/-]\d{4}$/.test(s)) return true;
  return false;
}

function isMeaningfulFactoryRow_(row) {
...
function extractDriverFareFromNotes_(value) {
  var text = String(value || "");
  var match = text.match(/(?:كروة\s*السائق|الكروة|الكروه|driver\s*fare)\s*[:：]?\s*([\d٠-٩,.]+)/i);
  if (!match) return 0;
  return toNumber_(match[1]);
}

function normalizeQuantityForMoney_(value) {
  var n = toNumber_(value);
  return n >= 1000 ? roundNumber_(n / 1000, 3) : n;
}
