/**
 * Tracking/GPS helpers kept in-repo so a clasp push does not wipe live GPS.
 * Uses SPREADSHEET_ID from Main.gs.
 */
...
function getDriversLive() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    drivers.push({
      ...
      tripStatus: String(data[i][6] || "no_trip"),
...
function getDriverWallet(data) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var i = 1; i < values.length; i++) {
      ...
      var rowQty = gpsToNumber_(row[5]);
      var rowLiters = gpsToNumber_(row[10]);
      var rowPrice = gpsToNumber_(row[13]);
  ...
  if (maintenanceSheet) {
    ...
    for (var j = 1; j < mValues.length; j++) {
      ...
      var cost = gpsToNumber_(mValues[j][8]);
