...
// Canonical storage is month sheets only: yyyy_MM and F_yyyy_MM.
var ENABLE_ROUTED_SUPPORT_SHEETS = false;
...
function handleRequest(e) {
  ...
  try {
    ...
    if (action === "companyVersionPolicy") {
      return json(getCompanyVersionPolicy(data));
    }

    if (action === "companyActivationVerify") {
      return json(verifyCompanyActivation(data));
    }

    if (action === "companyActivationList") {
      return json(listCompanyActivationCodes(data));
    }

    if (action === "companyActivationCreate") {
      return json(createCompanyActivationCode(data));
    }

    if (action === "companyActivationSetEnabled") {
      return json(setCompanyActivationEnabled(data));
    }

    if (action === "companyActivationUnbind") {
      return json(unbindCompanyActivationCode(data));
    }

    if (action === "companyActivationUpdateScope") {
      return json(updateCompanyActivationScope(data));
    }

    if (action === "companyActivationAuditList") {
      return json(listCompanyActivationAudit(data));
    }
    ...
    if (action === "cleanupSafetyPreview") {
      return json(cleanupSafetyPreview(data));
    }

    if (action === "cleanupSafetyApply") {
      return json(cleanupSafetyApply(data));
    }
    ...
    if (action === "systemHealthCheck") {
      return json(systemHealthCheck(data));
    }

    if (action === "createSystemBackup") {
      return json(createSystemBackup(data));
    }

    if (action === "resetMonthSheetsWithArchive") {
      return json(resetMonthSheetsWithArchive(data));
    }

    if (action === "cleanupRecentTestArtifacts") {
      return json(cleanupRecentTestArtifacts(data));
    }
    ...
    if (action === "repairFactorySheetLayout") {
      return json(repairFactorySheetLayout(data));
    }
    ...
    if (action === "trip") {
      return json(saveTripMain_(data));
    }

    if (action === "factory") {
      return json(saveFactoryMain_(data));
    }
...
function getCompanyVersionPolicy(data) {
  var versionCode = Number((data && data.versionCode) || 0);
  var policy = {
    enabled: true,
    minVersionCode: 4,
    latestVersionCode: 4,
    ...
    disabledMessage: "هذه النسخة من تطبيق مدير الحسابات غير مفعلة حاليًا. يرجى التواصل مع الإدارة لتفعيل نسخة معتمدة.",
    updateMessage: "هذه نسخة قديمة من تطبيق مدير الحسابات. يرجى تثبيت النسخة الجديدة المعتمدة من الإدارة."
  };
  ...
  if (!policy.enabled) {
    return {
      ...
      allowed: false,
      reason: "disabled",
      title: "تطبيق مدير الحسابات متوقف",
      message: policy.disabledMessage,
  ...
  if (!versionCode || versionCode < policy.minVersionCode) {
    return {
      success: true,
      allowed: false,
      reason: "update_required",
      title: "يجب تحديث تطبيق مدير الحسابات",
  ...
  return {
    ...
    reason: "allowed",
    title: "نسخة مفعلة",
    message: "هذه النسخة مفعلة ومسموح لها بالعمل.",
    supportPhone: policy.supportPhone,
    latestVersionCode: policy.latestVersionCode,
    latestVersionName: policy.latestVersionName
  };
...
function verifyCompanyActivation(data) {
  ...
  var rawCode = String(data.activationCode || data.code || "").trim();
  var rawDeviceId = String(data.deviceId || "").trim();
  var packageName = String(data.packageName || "").trim();
  var versionName = String(data.versionName || "").trim();
  var appKey = normalizeActivationAppKey_(data.appKey || resolveActivationAppKeyFromPackage_(packageName));
  var requestedCompanyId = sanitizeActivationToken_(data.companyId || "");
  ...
  if (!rawDeviceId) {
    return {
      ...
      message: "معرّف الجهاز غير متوفر"
    };
  }

  var code = String(rawCode).replace(/\s+/g, "").toUpperCase();
  var deviceId = String(rawDeviceId).replace(/\s+/g, "");
  ...
  if (!values || values.length <= 1) {
    return {
      success: false,
      allowed: false,
      reason: "empty_activation_sheet",
      message: "قائمة أكواد التفعيل فارغة"
    };
  }
  ...
  for (var i = 1; i < values.length; i++) {
    var currentCode = String(values[i][0] || "").replace(/\s+/g, "").toUpperCase();
    if (currentCode === code) {
      rowIndex = i + 1;
      row = values[i];
      break;
  ...
  var enabledValue = String(row[1] || "1").trim().toLowerCase();
  var isEnabled = !(enabledValue === "0" || enabledValue === "false" || enabledValue === "off" || enabledValue === "no" || enabledValue === "disabled");

  var scopeAppKey = normalizeActivationAppKey_(row[8] || "") || "company";
  var scopeAllowedPackage = String(row[9] || "").trim();
  var scopeCompanyId = sanitizeActivationToken_(row[10] || "");
  var scopeMaxDevices = parseActivationMaxDevices_(row[11]);
  ...
  if (!isEnabled) {
    return {
      ...
      message: "كود التفعيل موقوف من الإدارة"
    };
  }

  if (scopeAppKey && appKey && scopeAppKey !== appKey) {
    return {
      success: false,
      allowed: false,
      reason: "app_not_allowed",
      message: "هذا الكود غير مصرح لهذا التطبيق"
    };
  }

  if (scopeAllowedPackage && packageName && scopeAllowedPackage !== packageName) {
    return {
      success: false,
      allowed: false,
      reason: "package_not_allowed",
      message: "الحزمة الحالية غير مسموح لها بهذا الكود"
    };
  }

  if (scopeCompanyId && requestedCompanyId && scopeCompanyId !== requestedCompanyId) {
    return {
      success: false,
      allowed: false,
      reason: "company_not_allowed",
      message: "هذا الكود غير مخصص لهذه الشركة"
    };
  }

  if (scopeMaxDevices <= 0) {
    return {
      success: false,
      allowed: false,
      reason: "invalid_scope",
      message: "إعدادات الكود غير صحيحة"
    };
  }

  var assignedDevice = String(row[2] || "").trim();
  var now = new Date();

  if (!assignedDevice) {
    sheet.getRange(rowIndex, 2, 1, 11).setValues([["1", deviceId, now, packageName, versionName, now, String(row[7] || ""), scopeAppKey, scopeAllowedPackage, scopeCompanyId, scopeMaxDevices]]);
    return {
      success: true,
      allowed: true,
      reason: "activated",
      message: "تم تفعيل هذا الجهاز بنجاح",
      code: code,
  ...
  if (assignedDevice !== deviceId) {
    return {
      ...
      message: "هذا الكود مستخدم على جهاز آخر ولا يمكن إعادة تفعيله"
    };
  }

  sheet.getRange(rowIndex, 7).setValue(now);
  if (packageName) sheet.getRange(rowIndex, 5).setValue(packageName);
  if (versionName) sheet.getRange(rowIndex, 6).setValue(versionName);

  return {
    success: true,
    allowed: true,
    reason: "already_active",
    message: "هذا الجهاز مفعل مسبقًا",
    code: code,
    boundDevice: deviceId,
...
function ensureCompanyActivationSheet_() {
  ...
  var name = "CompanyActivationCodes";
  ...
  var row1 = sheet.getRange(1, 1, 1, lastColumn).getValues()[0] || [];
  var firstHeader = String(row1[0] || "").trim().toLowerCase();
  if (firstHeader !== "code") {
  ...
  for (var i = 0; i < headers.length; i++) {
    if (String(row1[i] || "").trim().toLowerCase() !== headers[i]) {
...
function listCompanyActivationCodes(data) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    var enabledValue = String(row[1] || "1").trim().toLowerCase();
    var enabled = !(enabledValue === "0" || enabledValue === "false" || enabledValue === "off" || enabledValue === "no" || enabledValue === "disabled");
    var deviceId = String(row[2] || "").trim();

    rows.push({
      code: code,
      enabled: enabled,
      bound: !!deviceId,
      deviceIdMasked: deviceId ? maskDeviceId_(deviceId) : "",
      activatedAt: row[3] || "",
      packageName: String(row[4] || "").trim(),
      versionName: String(row[5] || "").trim(),
      lastSeenAt: row[6] || "",
      notes: String(row[7] || "").trim(),
      appKey: normalizeActivationAppKey_(row[8] || "") || "company",
      allowedPackage: String(row[9] || "").trim(),
      companyId: sanitizeActivationToken_(row[10] || ""),
      maxDevices: parseActivationMaxDevices_(row[11]),
      rowNumber: i + 1
    });
  }
...
function createCompanyActivationCode(data) {
  ...
  var requestedCode = String(data.code || "").trim().replace(/\s+/g, "").toUpperCase();
  var notes = String(data.notes || "").trim();
  var enabled = data.enabled == null ? true : String(data.enabled).toLowerCase() !== "false" && String(data.enabled) !== "0";
  var appKey = normalizeActivationAppKey_(data.appKey || "company") || "company";
  var allowedPackage = String(data.allowedPackage || "").trim();
  var companyId = sanitizeActivationToken_(data.companyId || "");
  var maxDevices = parseActivationMaxDevices_(data.maxDevices);

  var code = requestedCode || generateCompanyActivationCode_();
  ...
  var sheet = ensureCompanyActivationSheet_();
  var rowInfo = findCompanyActivationRowByCode_(sheet, code);
  if (rowInfo.rowNumber > 0) {
    return {
      success: false,
      message: "الكود موجود مسبقًا"
    };
  }
...
function setCompanyActivationEnabled(data) {
  data = data || {};
  var code = String(data.code || "").trim().replace(/\s+/g, "").toUpperCase();
  var enabled = String(data.enabled).toLowerCase() === "true" || String(data.enabled) === "1";
  ...
  sheet.getRange(rowInfo.rowNumber, 2).setValue(enabled ? "1" : "0");

  appendCompanyActivationAudit_({
    action: enabled ? "enable_code" : "disable_code",
    code: code,
    actor: data.actor || "admin",
    appKey: normalizeActivationAppKey_(rowInfo.row && rowInfo.row[8] || "") || "company",
    companyId: sanitizeActivationToken_(rowInfo.row && rowInfo.row[10] || ""),
    details: enabled ? "enabled" : "disabled"
  });

  return {
    success: true,
    message: enabled ? "تم تفعيل الكود" : "تم إيقاف الكود",
    code: code,
    enabled: enabled
  };
}
...
function unbindCompanyActivationCode(data) {
  ...
  sheet.getRange(rowInfo.rowNumber, 3, 1, 5).clearContent();
  ...
  return {
    success: true,
    message: "تم فك ارتباط الجهاز عن الكود",
    code: code
  };
}
...
function updateCompanyActivationScope(data) {
  ...
  var appKey = normalizeActivationAppKey_(data.appKey || "");
  var allowedPackage = String(data.allowedPackage || "").trim();
  var companyId = sanitizeActivationToken_(data.companyId || "");
  var maxDevices = parseActivationMaxDevices_(data.maxDevices);
  ...
  if (appKey) sheet.getRange(rowInfo.rowNumber, 9).setValue(appKey);
  sheet.getRange(rowInfo.rowNumber, 10).setValue(allowedPackage);
  sheet.getRange(rowInfo.rowNumber, 11).setValue(companyId);
  sheet.getRange(rowInfo.rowNumber, 12).setValue(maxDevices);

  appendCompanyActivationAudit_({
    action: "update_scope",
    code: code,
    actor: data.actor || "admin",
    appKey: appKey || normalizeActivationAppKey_(rowInfo.row && rowInfo.row[8] || "") || "company",
    companyId: companyId || sanitizeActivationToken_(rowInfo.row && rowInfo.row[10] || ""),
    details: "package=" + allowedPackage + ", maxDevices=" + String(maxDevices)
  });

  return {
    success: true,
    message: "تم تحديث نطاق الصلاحية للكود",
    code: code
  };
}

function findCompanyActivationRowByCode_(sheet, code) {
  var values = sheet.getDataRange().getValues();
  var target = String(code || "").trim().replace(/\s+/g, "").toUpperCase();
  for (var i = 1; i < values.length; i++) {
    var current = String(values[i][0] || "").trim().replace(/\s+/g, "").toUpperCase();
    if (current === target) {
      return {
        rowNumber: i + 1,
        row: values[i]
...
function generateCompanyActivationCode_() {
  var stamp = Utilities.formatDate(new Date(), "Asia/Baghdad", "yyMMddHHmmss");
  var random = Math.floor(Math.random() * 9000 + 1000);
  return "CMP-" + stamp + "-" + random;
}

function maskDeviceId_(deviceId) {
  var txt = String(deviceId || "").trim();
  if (txt.length <= 12) return txt;
  return txt.slice(0, 6) + "..." + txt.slice(-6);
}

function normalizeActivationAppKey_(value) {
  var v = String(value || "").trim().toLowerCase();
  if (!v) return "";
  if (v === "company" || v === "driver" || v === "gas" || v === "owner") return v;
  if (v === "شركات" || v === "company_app") return "company";
  if (v === "سائق" || v === "drivers") return "driver";
  if (v === "غاز") return "gas";
  if (v === "مالك") return "owner";
  return "";
}

function resolveActivationAppKeyFromPackage_(packageName) {
  var pkg = String(packageName || "").trim().toLowerCase();
  if (!pkg) return "";
  if (pkg.indexOf(".company") >= 0) return "company";
  if (pkg.indexOf(".gas") >= 0) return "gas";
  if (pkg.indexOf(".owner") >= 0) return "owner";
  return "driver";
}

function sanitizeActivationToken_(value) {
  var raw = String(value || "").trim().toUpperCase();
  if (!raw) return "";
  return raw.replace(/[^A-Z0-9_\-]/g, "").slice(0, 64);
}

function parseActivationMaxDevices_(value) {
  var n = Number(value);
...
function ensureCompanyActivationAuditSheet_() {
  var ss = openSpreadsheetWithRetry_();
  var name = "CompanyActivationAudit";
  var sheet = ss.getSheetByName(name);

function repairFactorySheetLayout(data) {
  data = data || {};
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var explicitSheetName = String(data.sheetName || data.sheet || "").trim();
  var monthKey = resolveMonthKey_(data.month || "");
  var sheetName = explicitSheetName || (monthKey ? ("F_" + monthKey) : "");

  if (!sheetName) {
    return {
      success: false,
      message: "يجب تحديد اسم الشيت أو الشهر",
      example: "sheetName=F_2026_07"
    };
  }
  ...
  if (!sheet) {
    return {
      success: false,
      message: "الشيت غير موجود",
      sheetName: sheetName
    };
  }

  var beforeLastRow = sheet.getLastRow();
  var beforeLastColumn = sheet.getLastColumn();
  normalizeFactorySheetLayout_(sheet, factoryHeaders_());

  return {
    success: true,
    message: "تم إصلاح ترتيب شيت المعامل بنجاح",
    sheetName: sheetName,
  ...
  var headers = ["timestamp", "action", "code", "actor", "app_key", "company_id", "details"];
  if (sheet.getLastRow() === 0) {
    sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    return sheet;
  }
  ...
  if (String(row1[0] || "").trim().toLowerCase() !== "timestamp") {
    sheet.insertRowBefore(1);
    sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  }
  return sheet;
...
function appendCompanyActivationAudit_(entry) {
  var sheet = ensureCompanyActivationAuditSheet_();
  sheet.appendRow([
    new Date(),
    String(entry && entry.action || "").trim(),
    String(entry && entry.code || "").trim(),
    String(entry && entry.actor || "admin").trim(),
    normalizeActivationAppKey_(entry && entry.appKey || "") || "company",
    sanitizeActivationToken_(entry && entry.companyId || ""),
    String(entry && entry.details || "").trim()
  ]);
}
...
function listCompanyActivationAudit(data) {
  ...
  for (var i = values.length - 1; i >= 1; i--) {
    ...
    rows.push({
      ...
      actor: String(row[3] || "").trim(),
      appKey: normalizeActivationAppKey_(row[4] || "") || "company",
      companyId: sanitizeActivationToken_(row[5] || ""),
...
function loginDriver(data) {
  ...
  var requestedPhone = normalizePhone_(data.phone || "");
  var requestedCar = normalizeVehicleNumber_(data.carNumber || data.car || "");
  ...
  var authorized = findAuthorizedDriver_(requestedName, requestedPhone, requestedCar);
...
function findAuthorizedDriver_(driverName, phone, carNumber) {
  var ensured = ensureAuthorizedDriversSheet_();
  var sheet = ensured.sheet;

  if (ensured.created) {
    return createBootstrapAuthorization_(sheet, driverName, phone, carNumber);
  }
  ...
  var colMap = buildColumnMap_(headerRow);
  var normalizedCar = normalizeVehicleNumber_(carNumber);
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    var rowCar = normalizeVehicleNumber_(rowCarRaw);

    if (rowCar === normalizedCar) {
      if (!isAccessEnabled_(rowActiveRaw)) {
        return {
          allowed: false,
...
function createBootstrapAuthorization_(sheet, driverName, phone, carNumber) {
...
function ensureAuthorizedDriversSheet_() {
  var ss = openSpreadsheetWithRetry_();
  var sheet = getFirstExistingSheet_(ss, [
    "AuthorizedDrivers",
    "Authorized Drivers",
    "DriversAccess",
    "DriverAccess",
...
function isAccessEnabled_(value) {
  ...
  return !disabled[text.replace(/\s+/g, "")];
}
...
function saveTripMain_(data) {
  ...
  try {
    ...
    var destination = String(data.station || data.destination || "").trim();
    var quantityNum = toNumber_(data.quantity);
    var litersNum = toNumber_(data.liters);
    var pricePerTon = PRICE_PER_TON_HALAFAYA;
    var netQty = roundNumber_(quantityNum - (quantityNum * DEDUCTION_RATE), 3);
    var gasCost = roundNumber_(litersNum * PRICE_PER_LITER, 0);
    var kroa = toNumber_(data.kroa || data.kirwa || data.fare || data.driverFare || 0);
    var transportAmount = roundNumber_(netQty * pricePerTon, 0);
    var driverDeclaredTripPrice = toNumber_(data.tripPrice || data.price || transportAmount);
    var period15 = getPeriodFromDate_(data.unloadDate || data.loadDate || sendTime);
    ...
    var tripRow = [
      ...
      gasCost,
      kroa,
      monthPolicy.monthKey,
      period15,
      "accepted",
      driverDeclaredTripPrice
    ];

    sheet.appendRow(tripRow);

    var routedStationSheet = "";
    if (ENABLE_ROUTED_SUPPORT_SHEETS) {
      routedStationSheet = mirrorTripToStationSheet_(ss, monthPolicy.monthKey, destination, tripRow);
    }
    ...
    return {
      ...
      sheetName: sheetName,
      routedSheetName: routedStationSheet || "",
...
function saveFactoryMain_(data) {
  ...
  try {
    ...
    var sendTime = Utilities.formatDate(new Date(), "Asia/Baghdad", "yyyy-MM-dd HH:mm:ss");
    var factoryNameValue = String(data.factory || data.destination || "").trim();
    var ownerValue = String(data.owner || data.ownerName || data.ownerType || "").trim();
    var vehicleOwnerValue = String(data.vehicleOwner || data.carOwner || ownerValue || "").trim();
    var loadDateValue = String(data.loadDate || data.unloadDate || "").trim();
    var unloadDateValue = String(data.unloadDate || data.loadDate || "").trim();
    var quantityNum = toNumber_(data.quantity);
    var pricePerTon = PRICE_PER_TON_FACTORY;
    var netQty = roundNumber_(quantityNum - (quantityNum * DEDUCTION_RATE), 3);
    var kroa = toNumber_(data.kroa || data.kirwa || data.fare || data.driverFare || 0);
    var transportAmount = roundNumber_(netQty * pricePerTon, 0);
    var period15 = getPeriodFromDate_(data.unloadDate || data.loadDate || sendTime);
    ...
    sheet.appendRow(factoryRow);

    var routedFactorySheet = "";
    if (ENABLE_ROUTED_SUPPORT_SHEETS) {
      routedFactorySheet = mirrorFactoryToPlantSheet_(ss, monthPolicy.monthKey, factoryNameValue, factoryRow);
    }
    ...
    return {
      ...
      sheetName: sheetName,
      routedSheetName: routedFactorySheet || "",
      tripMonth: monthPolicy.monthKey,
      sendTime: sendTime
    };
  } finally {
...
function getDriverWallet(data) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var i = startRow; i < values.length; i++) {
      ...
      var station = String(getCellByAliases_(row, colMap, ["station", "destination", "المحطة", "الوجهة", "المحطة/المعمل", "المحطةالمعمل"], 7) || "");
      var factory = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة", "المحطة/المعمل", "المحطةالمعمل"], /^f_/i.test(sheetName) ? 7 : -1) || "");
...
function getDriverTrips(data) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var i = startRow; i < values.length; i++) {
      ...
      var factory = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة", "المحطة/المعمل", "المحطةالمعمل"], -1) || "");
      var source = String(getCellByAliases_(row, colMap, ["source", "type", "rowtype"], -1) || "");
      var ownerValue = String(getCellByAliases_(row, colMap, ["owner", "ownertype", "المالك"], 6) || "").trim();
      if (!ownerValue) {
        ownerValue = String(getCellByAliases_(row, colMap, ["carowner", "vehicleowner", "مالكالسياره", "مالكالسيارة", "مالكالسيارةأوالمالك", "مالكالسيارهاوالمالك"], /^f_/i.test(sheetName) ? 10 : -1) || "").trim();
      }
      ...
      if (hasHeader && isFactoryContext) {
        ...
        if (quantityLooksUrl || mappedFactoryLooksDate || unloadLooksFactoryName) {
          ...
          var shiftedUnloadDate = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة", "المحطة/المعمل", "المحطةالمعمل"], 6) || "");
      ...
      trips.push({
        ...
        owner: String(isLegacyFactoryRow ? "" : ownerValue),
        destination: station,
        station: station,
        factory: factory,
        source: source,
...
function getAvailableMonths() {
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    // Keep only the monthly source tabs (not generated OFF/TPL/STMT sheets).
    if (!isSourceMonthSheetName_(sheetName)) continue;
...
function getAllReceiptsData(month) {
  ...
  for (var s = 0; s < targetSheets.length; s++) {
    ...
    for (var i = startRow; i < values.length; i++) {
      ...
      var source = String(getCellByAliases_(row, colMap, ["source", "type", "rowtype"], -1) || "");
      var destination = String(getCellByAliases_(row, colMap, ["station", "destination", "المحطة", "الوجهة", "المحطة/المعمل", "المحطةالمعمل"], 7) || "");
      var factory = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة", "المحطة/المعمل", "المحطةالمعمل"], /^f_/i.test(sheetName) ? 7 : -1) || "");
      ...
      var kroaValue = toNumber_(getCellByAliases_(row, colMap, ["kroa", "driverfare", "fare", "الكروة"], isFactory ? 18 : 21));
      var driverTripPrice = toNumber_(getCellByAliases_(row, colMap, ["drivertripprice", "tripprice", "سعرالنقلةالمذكورمنالسائق", "سعرالنقلة"], 25));
      ...
      if (!String(loadDate || "").trim() && String(unloadDate || "").trim()) {
        loadDate = unloadDate;
      }
      var ownerValue = String(getCellByAliases_(row, colMap, ["owner", "ownertype", "المالك"], 6) || "").trim();
      if (!ownerValue) {
      ...
      result.push({
        ...
        driverTripPrice: driverTripPrice,
        price: finalAmount,
        gasCost: gasCost,
        kroa: kroaValue,
...
function ensureFactorySheet_(ss, sheetName) {
  ...
  normalizeFactorySheetLayout_(sheet, headers);
...
function normalizeFactorySheetLayout_(sheet, headers) {
  headers = headers || factoryHeaders_();
  ...
  var hasHeader = isLikelyHeaderRow_(headerRow);
  var colMap = hasHeader ? buildColumnMap_(headerRow) : {};
  var startRow = hasHeader ? 2 : 1;
  var dataRows = lastRow >= startRow ? sheet.getRange(startRow, 1, lastRow - startRow + 1, lastColumn).getValues() : [];
  var normalizedRows = [];

  for (var i = 0; i < dataRows.length; i++) {
    var row = dataRows[i] || [];
    if (isRowEffectivelyEmpty_(row)) continue;
    normalizedRows.push(buildNormalizedFactoryRow_(row, colMap, sheet.getName()));
  }
...
function buildNormalizedFactoryRow_(row, colMap, sheetName) {
  var docNumber = String(getCellByAliases_(row, colMap, ["docnumber", "رقمالوصل"], 0) || "").trim();
  var driverName = String(getCellByAliases_(row, colMap, ["drivername", "driver", "السائق", "اسمالسائق"], 1) || "").trim();
  var carNumber = String(getCellByAliases_(row, colMap, ["carnumber", "car", "رقمالسيارة"], 2) || "").trim();
  var loadDate = String(getCellByAliases_(row, colMap, ["loaddate", "تاريخالتحميل"], 3) || "").trim();
  var unloadDate = String(getCellByAliases_(row, colMap, ["unloaddate", "تاريخالتفريغ"], 4) || "").trim();
  var quantity = toNumber_(getCellByAliases_(row, colMap, ["quantity", "qty", "الكمية"], 5));
  var owner = String(getCellByAliases_(row, colMap, ["owner", "ownertype", "المالك"], -1) || "").trim();
  var factory = String(getCellByAliases_(row, colMap, ["factory", "اسم_المعمل", "اسم_الجهة", "الجهة", "المحطة/المعمل", "المحطةالمعمل"], 7) || "").trim();
  var imageUrl = String(getCellByAliases_(row, colMap, ["imageurl", "رابطالصورة"], 8) || "").trim();
  var sendTime = String(getCellByAliases_(row, colMap, ["timestamp", "sendtime", "وقتالارسال"], 9) || "").trim();
  var vehicleOwner = String(getCellByAliases_(row, colMap, ["carowner", "vehicleowner", "مالكالسياره", "مالكالسيارة", "مالكالسيارةأوالمالك", "مالكالسيارهاوالمالك"], 10) || owner).trim();
  var notes = String(getCellByAliases_(row, colMap, ["notes", "ملاحظات"], 11) || "").trim();
  var normalizedDocNumber = String(getCellByAliases_(row, colMap, ["normalizeddocnumber", "رقمالوصلالموحد"], 12) || normalizeDocNumber_(docNumber)).trim();
  var source = String(getCellByAliases_(row, colMap, ["source"], 13) || "factory").trim() || "factory";
  var movementType = String(getCellByAliases_(row, colMap, ["نوعالحركة", "type", "rowtype"], 14) || source).trim() || source;
  var direction = String(getCellByAliases_(row, colMap, ["الجهة", "destination", "station", "factory"], 15) || factory).trim() || factory;
  var pricePerTon = toNumber_(getCellByAliases_(row, colMap, ["priceperton", "سعرالطنالمعتمد"], 16) || PRICE_PER_TON_FACTORY);
  var netQty = toNumber_(getCellByAliases_(row, colMap, ["netquantity", "الكميةالمحتسبة"], 17) || roundNumber_(quantity - (quantity * DEDUCTION_RATE), 3));
  var gasCost = toNumber_(getCellByAliases_(row, colMap, ["gascost", "قيمةالكاز"], 18) || 0);
  var kroa = toNumber_(getCellByAliases_(row, colMap, ["kroa", "الكروة"], 19) || 0);
  var month = String(getCellByAliases_(row, colMap, ["month", "الشهر"], 20) || extractMonthKeyFromSheetName_(sheetName || "") || "").trim();
  var period15 = toNumber_(getCellByAliases_(row, colMap, ["period15", "فترةالشهر"], 21) || getPeriodFromDate_(unloadDate || loadDate || sendTime));
  var verificationStatus = String(getCellByAliases_(row, colMap, ["verificationstatus", "حالةالتحقق"], 22) || "accepted").trim() || "accepted";
  var transportAmount = toNumber_(getCellByAliases_(row, colMap, ["price", "transportamount", "سعرالنقل"], 23) || roundNumber_(netQty * pricePerTon, 0));
...
function resetMonthSheetsWithArchive(data) {
  var monthKey = resolveMonthKey_((data && data.month) || buildMonthKey_(new Date()));
  if (!monthKey) {
    return { success: false, message: "month مطلوب بصيغة yyyy_MM" };
  }

  var scope = String((data && data.scope) || "all").toLowerCase();

  var ss = openSpreadsheetWithRetry_();
  var stamp = Utilities.formatDate(new Date(), "Asia/Baghdad", "yyyyMMdd_HHmmss");
  var targetNames = [monthKey, "F_" + monthKey];
  if (scope === "trip") {
    targetNames = [monthKey];
  } else if (scope === "factory") {
    targetNames = ["F_" + monthKey];
  }
  var archived = [];
  var recreated = [];

  for (var i = 0; i < targetNames.length; i++) {
    var srcName = targetNames[i];
    var src = ss.getSheetByName(srcName);

    if (src) {
      var archName = "ARCH_" + srcName + "_" + stamp;
      src.setName(archName);
      archived.push(archName);
    }

    if (srcName.indexOf("F_") === 0) {
      ensureFactorySheet_(ss, srcName);
    } else {
      ensureTripsSheet_(ss, srcName);
    }
    recreated.push(srcName);
  }

  return {
    success: true,
    message: "تمت أرشفة وإعادة إنشاء شيتات الشهر بنجاح",
    month: monthKey,
    archivedSheets: archived,
    recreatedSheets: recreated
  };
}

function mirrorTripToStationSheet_(ss, monthKey, stationName, tripRow) {
  if (!ss || !tripRow || !tripRow.length) return "";

  var sheetName = buildStationRouteSheetName_(stationName, monthKey);
  if (!sheetName) return "";

  var sheet = ensureRoutedSheet_(ss, sheetName, tripHeaders_());
  sheet.appendRow(tripRow);
  return sheetName;
}

function mirrorFactoryToPlantSheet_(ss, monthKey, factoryName, factoryRow) {
  if (!ss || !factoryRow || !factoryRow.length) return "";

  var sheetName = buildFactoryRouteSheetName_(factoryName, monthKey);
  if (!sheetName) return "";

  var sheet = ensureRoutedSheet_(ss, sheetName, factoryHeaders_());
  sheet.appendRow(factoryRow);
  return sheetName;
}

function ensureRoutedSheet_(ss, sheetName, headers) {
  var sheet = ss.getSheetByName(sheetName);
  if (!sheet) {
    sheet = ss.insertSheet(sheetName);
...
function buildStationRouteSheetName_(stationName, monthKey) {
  var code = buildShortArabicCode_(stationName, 6, "عام");
  var tag = buildShortMonthTag_(monthKey);
  return "مح_" + code + "_" + tag;
}

function buildFactoryRouteSheetName_(factoryName, monthKey) {
  var code = buildShortArabicCode_(factoryName, 6, "عام");
  var tag = buildShortMonthTag_(monthKey);
  return "مع_" + code + "_" + tag;
}

function buildShortMonthTag_(monthKey) {
  var key = resolveMonthKey_(monthKey);
  if (!key) return "0000";
  var parts = key.split("_");
  var year = String(parts[0] || "0000");
  var month = String(parts[1] || "00");
  return year.slice(-2) + month;
}

function buildShortArabicCode_(name, maxLen, fallbackValue) {
  var s = String(name || "").trim();
  s = s.replace(/[أإآ]/g, "ا").replace(/ة/g, "ه").replace(/ى/g, "ي");
  s = s.replace(/محطه|محطة|معمل|ال|شركه|شركة/g, " ");
  s = s.replace(/[^\u0600-\u06FF0-9\s]/g, " ").replace(/\s+/g, " ").trim();

  if (!s) return String(fallbackValue || "عام");

  var compact = s.replace(/\s+/g, "");
  var limit = Number(maxLen || 6);
  if (limit <= 0) limit = 6;
...
function getCurrentTripsSheet_(ss) {
  ...
  for (var i = 0; i < allSheets.length; i++) {
    ...
    if (!isSourceMonthSheetName_(n)) continue;
...
function getMonthSheets_(ss, month) {
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    if (!isSourceMonthSheetName_(name)) continue;

    var foundKey = extractMonthKeyFromSheetName_(name);
    if (foundKey === key) {
...
function isSourceMonthSheetName_(name) {
  var s = String(name || "").trim();
  return /^(F_)?\d{4}_\d{2}$/i.test(s);
}
...
function cleanupSafetyPreview(data) {
  try {
    ...
    if (monthKey) {
      var generatedPrefixes = ["TPL_" + monthKey + "_", "STMT_" + monthKey + "_"];
      for (var i = 0; i < sheets.length; i++) {
        var gName = String(sheets[i].getName() || "");
        for (var gp = 0; gp < generatedPrefixes.length; gp++) {
          if (gName.indexOf(generatedPrefixes[gp]) === 0) {
            generatedCandidates.push(gName);
            break;
          }
        }
      }
    ...
    var routedEmptyCandidates = [];
    var testRowsBySheet = [];
    var testDocRegex = /^(CHK|TST|RTST|RFCT|TSTST|TSTFC)/i;
    var testTextRegex = /(اختبار|test|^drv\d*$)/i;

    for (var s = 0; s < sheets.length; s++) {
      var sh = sheets[s];
      var sheetName = String(sh.getName() || "").trim();

      if (/^(مح_|مع_)/.test(sheetName) && sh.getLastRow() <= 1) {
        routedEmptyCandidates.push(sheetName);
      }
      ...
      for (var r = 1; r < vals.length; r++) {
        var row = vals[r] || [];
        var doc = String(row[0] || "").trim();
        var driver = String(row[1] || "").trim();
        var notes = String(row[14] || row[7] || "").trim();
        if (testDocRegex.test(doc) || testTextRegex.test(driver) || testTextRegex.test(notes)) {
          hits++;
        }
      }

      if (hits > 0) {
        testRowsBySheet.push({ sheetName: sheetName, testRowsCount: hits });
      }
    }

    generatedCandidates.sort();
    routedEmptyCandidates.sort();
    testRowsBySheet.sort(function(a, b) {
    ...
    return {
      ...
      routedEmptyCandidates: routedEmptyCandidates,
      testRowsBySheet: testRowsBySheet,
      summary: {
        generatedSheetsCount: generatedCandidates.length,
        routedEmptySheetsCount: routedEmptyCandidates.length,
        sourceSheetsWithTestRows: testRowsBySheet.length
      },
      recommendation: "شغّل createSystemBackup أولاً، ثم cleanupSafetyApply مع backupFileId"
    };
  } catch (err) {
    return {
      success: false,
      message: "فشل معاينة التنظيف: " + String(err)
    };
  }
}
...
function cleanupSafetyApply(data) {
  try {
    var operation = String((data && data.operation) || "all").trim().toLowerCase();
    if (operation !== "generated" && operation !== "test" && operation !== "all") {
      operation = "all";
    }

    var backupCheck = validateBackupGuard_(data && data.backupFileId);
    ...
    var result = {
      success: true,
      message: "تم تنفيذ التنظيف الآمن بنجاح",
      backup: {
        backupFileId: backupCheck.backupFileId,
        backupName: backupCheck.backupName
      }
    };

    if (operation === "generated" || operation === "all") {
      result.generatedCleanup = cleanupGeneratedSheets({ month: data && data.month ? data.month : "" });
    }

    if (operation === "test" || operation === "all") {
      result.testArtifactsCleanup = cleanupRecentTestArtifacts({ dryRun: false });
    }
    ...
    return {
      success: false,
      message: "فشل التنفيذ الآمن للتنظيف: " + String(err)
    };
  }
}

function validateBackupGuard_(backupFileId) {
  var sourceId = String(SPREADSHEET_ID || "").trim();
  var backupId = String(backupFileId || "").trim();

  if (!backupId) {
    return {
      success: false,
      message: "backupFileId مطلوب للتنظيف الآمن. نفّذ createSystemBackup أولاً"
    };
  }
  ...
  if (backupId === sourceId) {
    return {
      success: false,
      message: "backupFileId يشير إلى الملف الأصلي وليس نسخة احتياطية"
    };
  }

  try {
    var backupFile = DriveApp.getFileById(backupId);
    var backupName = String(backupFile.getName() || "");
    if (backupName.indexOf("BACKUP_") !== 0) {
      return {
        success: false,
        message: "الملف المحدد لا يبدو نسخة احتياطية صحيحة (يجب أن يبدأ اسمه بـ BACKUP_)"
      };
    }
    ...
    return {
      success: false,
      message: "تعذر الوصول إلى backupFileId: " + String(err)
    };
  }
}
...
function cleanupRecentTestArtifacts(data) {
  var dryRun = String((data && data.dryRun) || "").toLowerCase() === "true";
  var ss = openSpreadsheetWithRetry_();
  var sheets = ss.getSheets();
  var deletedSheets = [];
  var scannedMonthSheets = [];
  var deletedRowsBySheet = [];

  var testDocRegex = /^(CHK|TST|RTST|RFCT|TSTST|TSTFC)/i;
  var testTextRegex = /(اختبار|test|^drv\d*$)/i;

  // Remove only routed short sheets that are empty (keep non-empty sheets for safety).
  for (var i = sheets.length - 1; i >= 0; i--) {
    var sh = sheets[i];
    var name = String(sh.getName() || "").trim();
    if (/^(مح_|مع_)/.test(name) && sh.getLastRow() <= 1) {
  ...
  // Remove test rows from monthly source sheets.
  sheets = ss.getSheets();
  for (var s = 0; s < sheets.length; s++) {
    var sheet = sheets[s];
    ...
    scannedMonthSheets.push(sheetName);
  ...
  deletedSheets.sort();
  deletedRowsBySheet.sort(function(a, b) {
    return String(a.sheetName).localeCompare(String(b.sheetName));
  });

  var totalRows = 0;
  for (var x = 0; x < deletedRowsBySheet.length; x++) {
    totalRows += Number(deletedRowsBySheet[x].count || 0);
  }

  return {
    success: true,
    dryRun: dryRun,
    message: dryRun ? "معاينة تنظيف عناصر الاختبار" : "تم تنظيف عناصر الاختبار بنجاح",
    deletedSheets: deletedSheets,
    deletedSheetsCount: deletedSheets.length,
    deletedRowsBySheet: deletedRowsBySheet,
    deletedRowsCount: totalRows,
    scannedMonthSheets: scannedMonthSheets
  };
}
...
function createSystemBackup(data) {
  try {
    ...
    var label = String((data && data.label) || "manual").trim();
    label = label ? label.replace(/\s+/g, "_") : "manual";

    var sourceFile = DriveApp.getFileById(ss.getId());
    var copyName = "BACKUP_" + ss.getName() + "_" + stamp + "_" + label;
    var copyFile = sourceFile.makeCopy(copyName);
    ...
    return {
      ...
      backupUrl: "https://docs.google.com/spreadsheets/d/" + copyFile.getId() + "/edit",
    ...
    return {
      success: false,
      message: "فشل إنشاء النسخة الاحتياطية: " + String(err)
    };
  }
}
...
function systemHealthCheck(data) {
  try {
    var ss = openSpreadsheetWithRetry_();
    var month = resolveMonthKey_((data && data.month) || "");
    var templateName = String((data && data.templateSheetName) || OFFICIAL_TEMPLATE_SHEET_NAME || "").trim();
    var checks = [];
    var warnings = [];
    ...
    checks.push({
      ...
      ok: !!ss,
      info: ss ? (ss.getName() + " | " + ss.getId()) : "غير متاح"
    });

    var months = getAvailableMonths();
    checks.push({
      id: "months_available",
      ok: Array.isArray(months) && months.length > 0,
      info: "count=" + ((months && months.length) || 0)
    });

    if (templateName) {
      var t = ss.getSheetByName(templateName);
      checks.push({
        id: "official_template_sheet",
        ok: !!t,
        info: templateName
      });
      if (!t) {
        warnings.push("شيت القالب الرسمي غير موجود: " + templateName);
      }
    }
    ...
    if (month) {
      ...
      if (!monthSheet) {
        warnings.push("شيت الشهر المطلوب غير موجود: " + month);
      }
    }

    checks.push({
      id: "endpoint_build_exact",
      ok: typeof buildOfficialExactTemplate === "function",
      info: "buildOfficialExactTemplate()"
    });
    ...
    return {
      success: true,
      message: "تم فحص النظام",
      month: month || "",
      templateSheetName: templateName,
      checks: checks,
      warnings: warnings,
      summary: {
        passed: passed,
    ...
    return {
      success: false,
      message: "فشل فحص النظام: " + String(err)
    };
  }
}
...
function getTemplateRecordsFast_(monthKey, ss) {
  ...
  for (var i = startRow; i < values.length; i++) {
    ...
    result.push({
      ...
      destination: String(getCellByAliases_(row, colMap, ["station", "destination", "المحطة", "الوجهة", "المحطة/المعمل", "المحطةالمعمل"], 7) || ""),
